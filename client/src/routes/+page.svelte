<script lang="ts">
	...
	function getTransactionAmounts(transaction: Transaction): Map<AssetId, bigint> {
		const amounts = new Map<AssetId, bigint>();

		for (const { asset, amount } of transaction.inputs) {
			amounts.set(asset, (amounts.get(asset) ?? 0n) - amount);
		}

		for (const { asset, amount } of transaction.outputs) {
			amounts.set(asset, (amounts.get(asset) ?? 0n) + amount);
		}
		...
		{#if transactions.length > 0}
			{#each transactions as transaction}
				<a
					...
					aria-label="View transaction from {formatDate(transaction.createdAt)}"
					class="flex w-full flex-col gap-2 rounded-md border border-neutral-200 p-2 transition hover:bg-neutral-50 focus-visible:ring-2 focus-visible:ring-neutral-400 focus-visible:outline-none dark:border-neutral-700 dark:hover:bg-neutral-900"
				>
					{#each getTransactionAmounts(transaction) as [assetId, amount] (assetId)}
						{@const asset = ASSETS[assetId]}
						<div class="flex items-center justify-between gap-2">
							<div class="flex items-center gap-2">
								<img alt={asset.name} class="h-8 w-8" src={asset.icon} />
								<div>
									<p>{asset.name}</p>
									<p class="text-xs text-neutral-500">{amount < 0n ? "Sent" : amount > 0n ? "Received" : "No change"}</p>
								</div>
							</div>
							<p class="font-mono" class:text-red-500={amount < 0n} class:text-green-500={amount > 0n}>
								{amount < 0n ? "-" : amount > 0n ? "+" : ""}{formatAmount(amount < 0n ? -amount : amount, assetId)}
							</p>
						</div>
					{/each}
					<div>
