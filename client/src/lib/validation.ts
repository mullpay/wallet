...
const REGEX_BTC_TRANSACTION_ID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const REGEX_ASSET_TRANSACTION_ID = /^[0-9a-f]{64}$/i;

export type AddressType = "SPARK" | "LIGHTNING" | "ON-CHAIN";
export type TransactionIdType = "BTC" | "ASSET";
...
export function identifyTransactionId(id: string): TransactionIdType | undefined {
	if (REGEX_BTC_TRANSACTION_ID.test(id)) return "BTC";
	if (REGEX_ASSET_TRANSACTION_ID.test(id)) return "ASSET";
